...
export default function useSendFileMessageCallback(
  ...
  const sendMessage = useCallback(
    async (compressedFile: File, quoteMessage?: SendableMessageType) => {
      // Create FileMessageParams
      let params = await onBeforeSendFileMessage?.(compressedFile, quoteMessage);
      if (!params) {
      ...
      return new Promise<FileMessage>((resolve, reject) => {
        ...
        currentGroupChannel?.sendFileMessage(params as FileMessageCreateParams)
